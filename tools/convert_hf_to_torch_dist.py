...
def add_convertion_args(parser):
    ...
    try:
        ...
        pass
    parser.add_argument("--nla-model-is-critic", action="store_true",
                        help="Swap output_layer→[h,h]+Identity after mbridge load so torch_dist "
                             "matches NLAMegatronActor's critic-mode model_provider build.")
    return parser
...
def main():
    ...
    if args.nla_model_is_critic:
        ...
        for chunk in model:
            inner = unwrap_model(chunk)
            if getattr(inner, "post_process", False):
                d = inner.config.hidden_size
                with torch.no_grad():
                    inner.output_layer.weight.copy_(
                        torch.eye(d, dtype=inner.output_layer.weight.dtype,
                                  device=inner.output_layer.weight.device)
                    )
                print(f"NLA critic: provider built [{d},{d}] output_layer; set weight=eye")
